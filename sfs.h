...
/****************************************************************************
* prints data of given file if it exist, if not then Error message
*
****************************************************************************/
...
void display(char *fname) {
	...
	for (i=0; i<3; i++) {
		...
		for (j=0; j<4; j++) {
			...
			if (_inode_table[e_inode].TT[0]=='F')  { // entry is for a file
				if(strcmp(fname,_directory_entries[j].fname)==0)
					{
						char buff[1024];
						if(strcmp(_inode_table[e_inode].XX,"00")!=0)		// directory is just created; so no blocks assigned to it yet 
						{
							readSFS( stoi(_inode_table[e_inode].XX,2),buff);
							printf("%s\n",buff);
						}
						/*if(strcmp(_inode_table[e_inode].YY,"00")!=0)		
						{
							readSFS(stoi(_inode_table[e_inode].YY,2),buff);
							printf("%s",buff);
						}
						if(strcmp(_inode_table[e_inode].ZZ,"00")!=0)
						{
							readSFS(stoi(_inode_table[e_inode].ZZ,2),buff);
							printf("%s\n",buff);
						}*/
						flag=0;
					}
	...
	if(flag)
	printf("Error : %s file does not exits.\n",fname);
}
		...
		for(i=0;i<3 && exitflag==0;i++)
		{
			...
			else
			{
		    	...
		    	do
		    	{
		        	...
		        	if(cnt==1024 || count==3072)
		        		exitflag=1;
		    	}
		    	while(exitflag != 1);
		    	count+=cnt;
		    	char temp[3];
		    	itos(temp,newblock,2);
		    	String[cnt]='\0';
		    	writeSFS(newblock,String);
		    	if(i==0)
					strncpy(_inode_table[empty_ientry].XX,temp,2);
				else if(i==1)
					strncpy(_inode_table[empty_ientry].YY,temp,2);
				else if(i==2)
					strncpy(_inode_table[empty_ientry].ZZ,temp,2);
			}
		}
...
void rm(char *name) {
	...
	int blocks[3];
	int subblock[3];
	_directory_entry _directory_entries[4],_subdirectory_entries[4];		
	...
	for (i=0; i<3; i++) {
		...
		for (j=0; j<4; j++) {
			if (_directory_entries[j].F=='0') continue;	// means unused entry
			if (strcmp(_directory_entries[j].fname,name)) continue;	
			found=1;
			e_inode = stoi(_directory_entries[j].MMM,3);	// this is the inode that has more info about this entry
			
			if (_inode_table[e_inode].TT[0]=='F')  { // entry is for a file
				int temp=(stoi(_inode_table[e_inode].XX,2));
				if(temp)
				{
					writeSFS(temp,NULL);
					returnBlock(temp);
				}
				temp=(stoi(_inode_table[e_inode].YY,2));
				if(temp)
				{
					writeSFS(temp,NULL);
					returnBlock(temp);
				}
				temp=(stoi(_inode_table[e_inode].ZZ,2));
				if(temp)
				{
					writeSFS(temp,NULL);
					returnBlock(temp);
				...
				int w;
				for(w=0;current_working_directory[w]!='\0';w++)
					temp[w]=current_working_directory[w];
				temp[w]='\0';
				int cdi=CD_INODE_ENTRY,k,l;
				subblock[0] = stoi(_inode_table[e_inode].XX,2);
				subblock[1] = stoi(_inode_table[e_inode].YY,2);
				subblock[2] = stoi(_inode_table[e_inode].ZZ,2);
				for (k = 0; k< 3; ++k)
				{
					if(subblock[k]==0) continue;
					readSFS(subblock[k],(char *)_subdirectory_entries);	// lets read a directory entry; notice the cast
					for ( l = 0; l < 4; ++l)
					{
						if (_subdirectory_entries[l].F=='0') continue;	// means unused entry
						cd(_directory_entries[j].fname);
						rm(_subdirectory_entries[l].fname);
						//strcpy(current_working_directory,temp);
						int w;
						for(w=0;current_working_directory[w]!='\0';w++)
						current_working_directory[w]=temp[w];
						current_working_directory[w]='\0';
						CD_INODE_ENTRY=cdi;	
					}
				}
				_directory_entries[j].F='0';
		...
		for (k = 0; k < 4; ++k)
				{if(_directory_entries[k].F!='0') yes=1;}
		if(!yes)
			returnBlock(blocks[i]);
	}	
	if(!found)
		printf("File/directory does not exists.\n");
}
